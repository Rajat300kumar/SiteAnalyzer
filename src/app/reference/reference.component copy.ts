import { Component, OnInit, Output, OnChanges, EventEmitter, ViewChild, forwardRef, ElementRef, SimpleChanges, input, computed, effect, inject} from '@angular/core';
...
export class ReferenceComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    /* console.log('selectedPno',this.selectedPno);

    // Check if the 'data' input property has changed
    console.log('changes',changes);
    if (changes.hasOwnProperty('config')) {
      const newData = changes['config'];
      // Do something with the new data
      console.log('New data received in child component:', newData);
    } */
  }
  // toolbarConfig = {
  //   left: 0, // 1, 2, 3
